...
rm -rf "$BUILD/classes" "$BUILD/dex" "$BUILD/gen" "$BUILD/compiled.flata" \
  ...
  "$BUILD/SendImageToCooLED-debug-unsigned.apk" "$BUILD/SendImageToCooLED-debug.apk"
...
"$BUILD_TOOLS/zipalign" -f 4 "$BUILD/unaligned.apk" "$BUILD/SendImageToCooLED-debug-unsigned.apk"
...
"$BUILD_TOOLS/apksigner" sign --ks "$KEYSTORE" --ks-pass pass:android --key-pass pass:android \
  --out "$BUILD/SendImageToCooLED-debug.apk" "$BUILD/SendImageToCooLED-debug-unsigned.apk"
"$BUILD_TOOLS/apksigner" verify --verbose "$BUILD/SendImageToCooLED-debug.apk"

cp "$BUILD/SendImageToCooLED-debug.apk" "$ROOT/../SendImageToCooLED Android.apk"
echo "$ROOT/../SendImageToCooLED Android.apk"
